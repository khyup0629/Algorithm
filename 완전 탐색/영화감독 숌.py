# 문제 이해를 제대로 못했었다. n번째 종말의 숫자는 n번째로 작은 종말의 숫자라고 했는데
# 종말의 숫자는 "666"이 적어도 1개 이상 들어가면 되는 것이었다.
# 즉, 오름차순으로 나열하면
...
cnt = 0
while True:
    number += 1  # 1부터 무한대로 탐색
    string = str(number)  # 문자열로 변환
    # 666이 연속으로 들어가면 카운트
    # ex) 1666666은 string.count("666") = 2
    if string.count("666") >= 1:
        cnt += 1
    if cnt == n:
        print(number)
        break
