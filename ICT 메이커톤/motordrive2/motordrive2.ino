...
void loop() {
  if(btSerial.available()) //Serial.available()은 시리얼버퍼를 검사
  {
    ...
    if (dis <= 300){ // drone come
      // door open
      digitalWrite(IN1,HIGH);
      digitalWrite(IN2,LOW); // L9110S
      delay(600); // door open time 고치기
      while (1){
        digitalWrite(TRIG,HIGH);
        delayMicroseconds(10);
        ...
        Serial.print(dis);
        Serial.println("mm");
        if (dis <= 300){ // still inner 200m
          digitalWrite(IN1,LOW);
          digitalWrite(IN2,LOW); // door stop
        }
        else{ // drone gone
          // door close
          digitalWrite(IN1,LOW);
