# 열을 고정하고 행끼리 비교하며 철자가 하나라도 다르면 ?를 달면 된다.
...
for j in range(len(pattern[0])):
    question = False  # '?'를 달아야 하는지 여부
    for i in range(n-1):  # 열을 고정하고 행을 바꿔가며 비교
        # 같은 열에서 문자가 하나라도 다르다면 '?'를 달아야함
        if pattern[i][j] != pattern[i+1][j]:
            question = True  # '?'를 달아야 함
            break
    if question:
        result += '?'
    else:
...
패턴에는 알파벳과 "." 그리고 "?"만 넣을 수 있다. 가능하면 ?을 적게 써야 한다. 그 디렉토리에는 검색 결과에 나온 파일만 있다고 가정하고, 
파일 이름의 길이는 모두 같다.
